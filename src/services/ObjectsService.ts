...
export class ObjectsService extends Construct implements IContainerService {
  ...
  static readonly DEFAULT_BACKUP_CONTAINER_PORT = 8080;

  readonly id: string;

  constructor(scope: Construct, id: string, private props: ObjectsServiceProps) {
    super(scope, id);
  ...
  bind(platform: ContainerServiceProps): void {
    ...
    platform.loadbalancer.getListerner().addTargets(`${this.id}-targets`, {
      ...
      healthCheck: {
        enabled: true,
        path: '/',
        port: isEc2 ? undefined : ObjectsService.DEFAULT_BACKUP_CONTAINER_PORT.toString(),
      },
      priority: priority,
      port: ObjectsService.DEFAULT_BACKUP_CONTAINER_PORT,
    });
  }
  ...
  private loadEnvironmentFromConfig(config: ObjectsServiceConfiguration) {
